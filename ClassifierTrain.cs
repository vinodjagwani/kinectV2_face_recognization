...
class ClassifierTrain : IDisposable
{
    ...
    public ClassifierTrain(string theLocation)
    {
        location = theLocation;
        _IsTrained = LoadTrainingData(theLocation);
    }
    #endregion
    ...
    public bool Retrain()
    {
        return _IsTrained = LoadTrainingData(location);
    }
    /// <summary>
    /// Retrains the recognizer witout resetting variables like recognizer type.
    /// Takes String input to a different location for training data.
                            ...
                            switch (xmlreader.Name)
                            {
                                ...
                                case "FILE":
                                    if (xmlreader.Read())
                                    {
                                        ...
                                        trainingImages.Add(new Image<Gray, byte>(location + xmlreader.Value.Trim()));
                                    }
                                    break;
                            }
                        }
